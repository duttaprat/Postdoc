...
print(tissue_name)
tissues_of_interest  = tissue_name.split("_")
print(tissues_of_interest)
no_of_iteration = 500
sample_size = 0.7
file_path = "/data/projects/Tissue_specific_data/subsample/Data/all_input_stratified.feather"
OUTPUT_PATH = "/data/projects/Tissue_specific_data/subsample/results/tissue_enhanced/statistical_tests/"+tissue_name
...
for i in range(no_of_iteration):
    ...
    print(stratified_df)
    print(tissues_of_interest)
    cat1_df = stratified_df[stratified_df['sample_class'].isin(tissues_of_interest)]
    print("--- %s seconds ---" % (time.time() - start_time)) 
    print(cat1_df)
    cat2_df = stratified_df[~stratified_df['sample_class'].isin(tissues_of_interest)]
    #df_all = stratified_df.merge(cat1_df.drop_duplicates(), how='left', indicator=True)
    print(cat2_df)
    print("--- %s seconds ---" % (time.time() - start_time)) 
    # print(df_all)
    ...
    # print("--- %s seconds ---" % (time.time() - start_time)) 
    # print(df1_only)
    # cat2_df = df1_only.drop('_merge', axis=1).reset_index(drop=True)
    # print("--- %s seconds ---" % (time.time() - start_time)) 
    # #cat2_df = stratified_df[stratified_df['sample_class']!=tissue_name]
    print(stratified_df.shape, cat1_df.shape , cat2_df.shape)
    exit()
