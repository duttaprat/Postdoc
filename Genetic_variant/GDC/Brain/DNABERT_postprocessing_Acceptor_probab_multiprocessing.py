...
def get_scores(args):
    df_data, region_data, score_columns = args
    df = pd.DataFrame(df_data)
    region = pd.Series(region_data)
    scores = []
...
for df in patient_dfs:
     # Prepare the arguments for the get_scores function
    args = [(df.to_dict('list'), region._asdict(), ['Ref_probab', 'Alt_probab', 'LogOddRatio']) for region in unique_patient_regions.itertuples(index=True)]

    # Create a pool of worker processes
    with Pool() as pool:
        # Use the pool's map method to apply the get_scores function to each set of arguments
        results = pool.map(get_scores, args)

    # Convert the results to a numpy array and transpose it to get the scores in the desired format
    scores_array = np.array(results).T

    # Use the scores_array to populate your data dictionaries
    for i, data in enumerate([data_ref, data_alt, data_log_odd]):
        for j, region in enumerate(unique_patient_regions.itertuples(index=False)):
            key = f'{region.chr}_{region.Acceptor_start}_{region.Acceptor_end}_{region.varinat_start}_{region.variant_end}'
            data[key].append(scores_array[i, j])
